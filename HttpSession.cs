...
namespace FCHHttp
{
    ...
    public class HttpSession {
        ...
        public string GetStringFromResponse(HttpWebResponse resp)
        {
            try
            {
                using (var stream = resp.GetResponseStream())
        ...
        public WebResponse GetWithData(string url, object data = null, string json = null, Dictionary<string, string> headers = null)
        {
            var request = MakeRequestDefault($"{url}", headers: headers);
            byte[] databytes = new byte[0];
            long datalen = 0;
